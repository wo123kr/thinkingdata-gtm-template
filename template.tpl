      ...
      "param": [
        {
          ...
          "value": {
            ...
            "listItem": [
              {
                "type": 1,
                "string": "https://unpkg.com/thinking-data-web@*"
              },
              {
                "type": 1,
                "string": "https://cdn.jsdelivr.net/npm/thinking-data-web@* "
              },
              {
                "type": 1,
                "string": "https://cdnjs.cloudflare.com/ajax/libs/thinking-data-web/*"
              }
            ]
          }
        }
...
function executeAction() {
  ...
  // 유저 속성 처리 (다른 액션과 함께 실행)
  if (data.enableUserProperties && actionType !== 'userProperty') {
...
function handleUserProperty(ta) {
  ...
  if (method === 'user_set' || method === 'user_set_once') {
    ...
    if (data.userPropertiesObject && data.userPropertiesObject !== '') {
      var objProps = safeJsonParse(data.userPropertiesObject);
      if (objProps) {
        for (var key in objProps) {
          if (objProps.hasOwnProperty && objProps.hasOwnProperty(key)) {
    ...
    if (data.userPropertiesAppend && data.userPropertiesAppend.length > 0) {
      for (var i = 0; i < data.userPropertiesAppend.length; i++) {
        ...
        if (prop.name && prop.values !== undefined && prop.values !== '') {
          // 쉼표로 구분된 값들을 배열로 변환
          var valuesArray = makeString(prop.values).split(',');
          var cleanArray = [];
    ...
    if (data.userPropertiesUnset && data.userPropertiesUnset !== '') {
      ...
      var propsToUnset = makeString(data.userPropertiesUnset).split(',');
      var cleanProps = [];
...
Created on 2025. 7. 16. 오후 10:36:08
